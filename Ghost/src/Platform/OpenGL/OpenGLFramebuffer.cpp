...
namespace Ghost
{
	...
	namespace Utils
	{
		...
		static GLenum GhostFBTextureFormatToGL(FramebufferTextureFormat format) {
			switch (format)
			{
	...
	void OpenGLFramebuffer::Invalidate() {
		...
		{
			GT_CORE_ASSERT(m_ColorAttachments.size() <= 4);
	...
	void OpenGLFramebuffer::Resize(uint32_t width, uint32_t height) {
		...
		{
			GT_CORE_WARN("Attempted to resize framebuffer to {0},{1}", width, height);
			return;
		}
	...
	int OpenGLFramebuffer::ReadPixel(uint32_t attachmentIndex, int x, int y) {
		GT_CORE_ASSERT(attachmentIndex < m_ColorAttachments.size());
	...
	void OpenGLFramebuffer::ClearAttachment(uint32_t attachmentIndex, int value) {
		...
		glClearTexImage(m_ColorAttachments[attachmentIndex], 0,
			Utils::GhostFBTextureFormatToGL(spec.TextureFormat), GL_INT, &value);
	}
}
