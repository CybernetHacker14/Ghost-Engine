...
namespace Ghost
{
	...
	OpenGLShader::OpenGLShader(const std::string& filepath) :
...

		{
			...
			CreateProgram();
			GT_CORE_WARN("Shader creation took {0} ms", timer.ElapsedMillis());
		}
	...
	std::unordered_map<GLenum, std::string> OpenGLShader::PreProcess(const std::string& source) {
		...
		while (pos != std::string::npos)
		{
			...
			GT_CORE_ASSERT(Utils::ShaderTypeFromString(type), "Invalid shader type specified");
	...
	void OpenGLShader::CompileOrGetVulkanBinaries(const std::unordered_map<GLenum, std::string>& shaderSources) {
		...
		for (auto&& [stage, source] : shaderSources)
		{
			...
			else
			{
				...
				{
					GT_CORE_ERROR(module.GetErrorMessage());
					GT_CORE_ASSERT(false);
				}
	...
	void OpenGLShader::CreateProgram() {
		...
		if (isLinked == GL_FALSE)
		{
			...
			glGetProgramInfoLog(program, maxLength, &maxLength, info.data());
			GT_CORE_ERROR("Shader linking failed ({0}):\n{1}", m_FilePath, info.data());
	...
	void OpenGLShader::Reflect(GLenum stage, const std::vector<uint32_t>& shaderData) {
		...
		GT_CORE_TRACE("OpenGLShader::Reflect - {0} {1}", Utils::GLShaderStageToString(stage), m_FilePath);
		GT_CORE_TRACE("    {0} uniform buffers", resources.uniform_buffers.size());
		GT_CORE_TRACE("    {0} resources", resources.sampled_images.size());

		GT_CORE_TRACE("Uniform buffers:");
		for (const auto& resource : resources.uniform_buffers)
		{
			...
			GT_CORE_TRACE("  {0}", resource.name);
			GT_CORE_TRACE("    Size = {0}", bufferSize);
			GT_CORE_TRACE("    Binding = {0}", binding);
			GT_CORE_TRACE("    Members = {0}", memberCount);
		}
	}
