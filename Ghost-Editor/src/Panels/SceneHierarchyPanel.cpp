...
namespace Ghost
{
	...
	static void DrawComponent(const std::string& name, Entity entity, UIFunction uiFunction) {
		...
		if (entity.HasComponent<T>())
		{
			...
			if (ImGui::Button(ICON_FA_BARS, ImVec2{ lineHeight, lineHeight }))
			{
		...
		if (ImGui::BeginPopup("AddComponent"))
		{
			...
			if (ImGui::MenuItem("Camera"))
			{
				if (!m_SelectionContext.HasComponent<CameraComponent>())
					...
					GT_CORE_WARN("This entity already has the Camera Component!");
			...
			if (ImGui::MenuItem("Sprite Renderer"))
			{
				if (!m_SelectionContext.HasComponent<SpriteRendererComponent>())
					...
					GT_CORE_WARN("This entity already has the Sprite Renderer Component!");
			...
			if (ImGui::BeginDragDropTarget())
			{
				if (const ImGuiPayload* payload = ImGui::AcceptDragDropPayload("CONTENT_BROWSER_ITEM"))
				{
					...
					if (texture->IsLoaded())
						...
						GT_WARN("Could not load texture {0}", texturePath.filename().string());
				}
